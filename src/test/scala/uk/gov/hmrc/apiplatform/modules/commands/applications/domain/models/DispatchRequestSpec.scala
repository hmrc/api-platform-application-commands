...
class DispatchRequestSpec extends ApplicationCommandBaseSpec {
  ...
  "DispatchRequest" should {
    ...
    "read from json" in {
      val jsonText =
        s"""{ "command": {"actor":{"email":"bob@example.com","actorType":"COLLABORATOR"},"collaborator":{"emailAddress":"alice@example.com","role":"DEVELOPER","userId":"${aUserId.value}"},"timestamp":"$nowAsText","updateType":"addCollaborator"}, "verifiedCollaboratorsToNotify": ["${anActorEmail.text}"] }"""
